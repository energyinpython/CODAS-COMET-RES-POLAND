...
def plot_boxenplot(data, coeff_name):
    ax = sns.boxenplot(x = 'variable', y = 'value', hue = r'$\tau$', data = data)
    ax.tick_params(axis='x', rotation = 90)
    ax.set_xlabel('Distance')
    ax.set_ylabel(r'$' + coeff_name + '$')
    ax.grid(True)
    ax.set_axisbelow(True)
    plt.legend(bbox_to_anchor=(0., 1.07, 1., .102), loc='center',
    ncol=5, borderaxespad=0., edgecolor = 'black', title = r'$\tau$ parameter')
    plt.tight_layout()
    plt.savefig('boxenplot_' + coeff_name + '.pdf')
    plt.show()
...
# range of tau threshold parameter
tau_vec = np.arange(0.01, 0.06, 0.01)

# r - num of rows, c - num of cols, r - 16 - num of alts
r, c = 16, len(data.min())

for it in range(iter):
    # generate decision matrix random data
    matrix = np.zeros((r, c))
    ...
    for met_dis in combinations_names:
        # CODAS
        for tau_v in tau_vec:
            pref1 = CODAS(matrix, weights, types, met_dis[0], tau_v)
            rankingPrep = np.argsort(-pref1)
            ranking1 = np.argsort(rankingPrep) + 1
    
            pref2 = CODAS(matrix, weights, types, met_dis[1], tau_v)
            ...
            results_dict_rw[r'$\tau$'].append(str(tau_v))
            ...
            results_dict_WS[r'$\tau$'].append(str(tau_v))

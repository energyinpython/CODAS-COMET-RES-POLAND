...
for it in range(0, 2):
    for method_distance in list_methods_distance:
        ...
        if it == 0:
            df_writer_pref[r'$C_{' + method_distance.__name__[:3] + '}$'] = pref
            df_writer_rank[r'$C_{' + method_distance.__name__[:3] + '}$'] = rank
        elif it == 1:
            df_writer_pref[r'$T_{' + method_distance.__name__[:3] + '}$'] = pref
            df_writer_rank[r'$T_{' + method_distance.__name__[:3] + '}$'] = rank
...
heatmap = sns.heatmap(df_new_heatmap_rw, annot=True, fmt=".4f", cmap="PuBuGn",
...
plt.figure(figsize = (7,4))
sns.set(font_scale=1.1)
heatmap = sns.heatmap(df_new_heatmap_ws, annot=True, fmt=".4f", cmap="PuBuGn",
                      linewidth=1, linecolor='w')
plt.yticks(va="center")
plt.tight_layout()
plt.show()
